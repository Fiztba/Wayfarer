...
app.whenReady().then(async () => {
  ...
  if (!baseline) {
    ...
    await waitFor(`document.querySelector('.map-confidence').textContent.includes('Best guess')`)
    assert.equal(await js(`document.querySelector('.map-zone-select').value`), 'maze', 'an unanchored cross-zone guess must not change the displayed zone')
    assert.ok(await js(`document.querySelector('.map-level').textContent.includes('L0')`))
    ...
    console.log('ok losing position and guessing in another zone preserves the view until explicit confirmation')
  }
