...
export const HELP_TOPICS: HelpTopic[] = [
  {
    id: 'copyover', title: 'Updates & Connection Copyover', blocks: [
      ...
      { p: 'Pause Triggers in the bottom-right corner temporarily pauses all triggers in the current session, including global, world, and character triggers. Triggers Paused indicates the pause is active; click it to resume. Individual enabled settings are preserved. Lines received while paused are not replayed. Aliases, macros, and timers keep working. The pause survives a connection-preserving update, but starts off in a newly opened session.' },
    ...
    blocks: [
      ...
      },
      {
        list: [
          ...
          'The ⌗ toolbar button (or #map tidy) lays the current zone out again from its exits. Rooms are placed where their exits say, never on top of another room or across a link, and every room keeps its level. Only coordinates change; links are untouched, and #map untidy (or the Undo button) puts everything back. The layout it keeps is the one that would draw best: fewest links routed around a room, crossing another link, drawn longer than a step, or pointing away from their compass direction — a polish pass then nudges rooms one cell at a time, starting from both the fresh layout and the one already there, so a hand-tidied zone is improved rather than replaced. If no layout lies less than the current one — some areas are simply not flat — it says so and leaves the zone alone; "Apply anyway" or #map tidy! uses the best attempt regardless.',
          'Bent connections leave and enter through their recorded compass directions and route through gaps between rooms. A north exit can return through east: neither the line nor the inspector assumes an opposite return. Double ticks mean one exit drawn long; a filled arrow means no return exit is currently mapped. Crossings do not connect rooms. Purple links have a compass bearing that differs from their room placement.',
          'Exits in the toolbar opens an inspector for the selected room, or your current room. Click a row to trace that connection and dim the others; click it again to clear the trace. Every command stays listed, even when several lead to the same room. The inspector shows destination names, doors, actual return commands, and other zones/floors. The locate button shows the destination without walking. Collapse Exits to give the canvas more space.'
        ]
      },
      { h: 'Fixing mistakes' },
      {
