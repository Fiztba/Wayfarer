    ...
    blocks: [
      ...
      },
      {
        list: [
          ...
          'Links the grid cannot draw straight: where a link would run through another room it is wired around it along the gaps between rooms, with chevrons marking that it is one passage; a link drawn away from its compass direction carries a small arrow at each room pointing the way that exit really goes; two rooms on one level joined by up or down are drawn as a link with the ▲/▼ glyph on it.'
        ]
      },
      { h: 'Fixing mistakes' },
      {
